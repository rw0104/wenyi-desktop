...
| ≥ 1440px | 内容区放宽到 84rem；书架封面加大，列数增加 |
...
100% / 125% / 150% 三档字号下渲染真实界面，由页面自己报告是否出现横向溢出与越界元素，
并打印断点应当改变的量（导航栏方向、封面宽度、书架列数）：

```powershell
.\scripts\check_layout.ps1
```

字号那一档不是凑数。布局全部用 `rem`，所以用户把系统字号调大时界面应该整体变大，而不是撑破窗口——
这一档确实抓到过一个真问题：**150% 字号 + 760×560 最小窗口时横向溢出 15px，4 个元素被切掉**。

成因是两层叠加：网格项默认 `min-width: auto`，会被自身的 min-content 顶住；而文件路径那一行是
`white-space: nowrap`，且 flex 容器的 min-content 会把各子项的 min-content **相加**——
所以 `min-width: 0` 写在子项上并不能阻止这件事。于是一整条路径的宽度顶穿了外层网格的列宽。
给 `.panel` / `.card` 加上 `min-width: 0` 后归一，15 档全部 0 溢出。
